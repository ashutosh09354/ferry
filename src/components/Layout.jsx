...
export function Header(){
  ...
  return <header className="sticky top-0 z-50 border-b border-slate-100 bg-white/95 backdrop-blur">
    <div className="container-page flex min-h-20 items-center justify-between gap-5">
      ...
      <div className="hidden items-center gap-2 lg:flex"><button onClick={()=>go('/bookings')} className="btn-secondary !border-0">My Bookings</button><button onClick={()=>go('/login')} className="btn-secondary">Login</button><button onClick={()=>go('/schedule')} className="btn-primary">Book a Ferry <ArrowRight size={16}/></button></div>
      <div className="flex items-center gap-2 lg:hidden"><button onClick={()=>go('/login')} className="btn-secondary px-3 py-2 text-sm">Login</button><button onClick={()=>setOpen(!open)} className="rounded-lg p-2" aria-label="Toggle menu">{open?<X/>:<Menu/>}</button></div>
    </div>
    {open&&<div className="border-t bg-white p-5 lg:hidden"><div className="flex flex-col gap-2 text-sm font-semibold">{[['Routes','/routes'],['Schedule','/schedule'],['Experiences','/experiences'],['Help','/help']].map(([label,path])=><button key={path} className="p-3 text-left" onClick={()=>go(path)}>{label}</button>)}<button className="btn-secondary mt-2" onClick={()=>go('/bookings')}>My Bookings</button><button className="btn-primary" onClick={()=>go('/search')}>Book a Ferry</button></div></div>}
  </header>
}
